...
if [ "$MQTT_ADV_INFO_GLOBAL_ENABLE" == "yes" ]; then
    ...
    CONTENT='{"device":{"identifiers":["'$IDENTIFIERS'"],"manufacturer":"'$MANUFACTURER'","model":"'$MODEL'","name":"'$NAME'","sw_version":"'$SW_VERSION'"}, "'$RETAIN'", "icon":"mdi:ip","json_attributes_topic":"'$MQTT_PREFIX'/'$MQTT_ADV_INFO_GLOBAL_TOPIC'","state_topic":"'$MQTT_PREFIX'/'$MQTT_ADV_INFO_GLOBAL_TOPIC'","name":"'$UNIQUE_NAME'","unique_id":"'$UNIQUE_ID'","value_template":"{{ value_json.local_ip }}"}'
    $YI_HACK_PREFIX/bin/mosquitto_pub -i $HOSTNAME $RETAIN -h $HOST -t $TOPIC -m "$CONTENT"
    #Netmask
    ...
    CONTENT='{"device":{"identifiers":["'$IDENTIFIERS'"],"manufacturer":"'$MANUFACTURER'","model":"'$MODEL'","name":"'$NAME'","sw_version":"'$SW_VERSION'"}, "'$RETAIN'", "icon":"mdi:wifi","json_attributes_topic":"'$MQTT_PREFIX'/'$MQTT_ADV_INFO_GLOBAL_TOPIC'","state_topic":"'$MQTT_PREFIX'/'$MQTT_ADV_INFO_GLOBAL_TOPIC'","name":"'$UNIQUE_NAME'","unique_id":"'$UNIQUE_ID'","value_template":"{{ value_json.wlan_essid }}"}'
    $YI_HACK_PREFIX/bin/mosquitto_pub -i $HOSTNAME $RETAIN -h $HOST -t $TOPIC -m "$CONTENT"
    #Mac Address
    ...
    CONTENT='{"device":{"identifiers":["'$IDENTIFIERS'"],"manufacturer":"'$MANUFACTURER'","model":"'$MODEL'","name":"'$NAME'","sw_version":"'$SW_VERSION'"}, "'$RETAIN'", "icon":"mdi:webcam","json_attributes_topic":"'$MQTT_PREFIX'/'$MQTT_ADV_INFO_GLOBAL_TOPIC'","state_topic":"'$MQTT_PREFIX'/'$MQTT_ADV_INFO_GLOBAL_TOPIC'","name":"'$UNIQUE_NAME'","unique_id":"'$UNIQUE_ID'","value_template":"{{ value_json.serial_number }}"}'
    $YI_HACK_PREFIX/bin/mosquitto_pub -i $HOSTNAME $RETAIN -h $HOST -t $TOPIC -m "$CONTENT"
else 
    for ITEM in hostname local_ip netmask gateway wlan_essid mac_addr home_version fw_version model_suffix serial_number; do
        ...
        $YI_HACK_PREFIX/bin/mosquitto_pub -i $HOSTNAME $RETAIN -h $HOST -t $TOPIC -n
    done
fi
if [ "$MQTT_ADV_TELEMETRY_ENABLE" == "yes" ]; then
    ...
    CONTENT='{"device":{"identifiers":["'$IDENTIFIERS'"],"manufacturer":"'$MANUFACTURER'","model":"'$MODEL'","name":"'$NAME'","sw_version":"'$SW_VERSION'"}, "'$RETAIN'","device_class":"timestamp","icon":"mdi:timer-outline","json_attributes_topic":"'$MQTT_PREFIX'/'$MQTT_ADV_TELEMETRY_TOPIC'","state_topic":"'$MQTT_PREFIX'/'$MQTT_ADV_TELEMETRY_TOPIC'","name":"'$UNIQUE_NAME'","'unique_id'":"'$UNIQUE_ID'","value_template":"{{ (as_timestamp(now())-(value_json.uptime|int))|timestamp_local }}"}'
    $YI_HACK_PREFIX/bin/mosquitto_pub -i $HOSTNAME $RETAIN -h $HOST -t $TOPIC -m "$CONTENT"
    #WLanStrenght
    ...
    CONTENT='{"device":{"identifiers":["'$IDENTIFIERS'"],"manufacturer":"'$MANUFACTURER'","model":"'$MODEL'","name":"'$NAME'","sw_version":"'$SW_VERSION'"}, "'$RETAIN'","device_class":"signal_strength","icon":"mdi:wifi","json_attributes_topic":"'$MQTT_PREFIX'/'$MQTT_ADV_TELEMETRY_TOPIC'","state_topic":"'$MQTT_PREFIX'/'$MQTT_ADV_TELEMETRY_TOPIC'","name":"'$UNIQUE_NAME'","unique_id":"'$UNIQUE_ID'","value_template":"{{ ((value_json.wlan_strength|int) * 100 / 70 )|int }}","unit_of_measurement":"%"}'
    $YI_HACK_PREFIX/bin/mosquitto_pub -i $HOSTNAME $RETAIN -h $HOST -t $TOPIC -m "$CONTENT"
else 
...
CONTENT='{"device":{"identifiers":["'$IDENTIFIERS'"],"manufacturer":"'$MANUFACTURER'","model":"'$MODEL'","name":"'$NAME'","sw_version":"'$SW_VERSION'"}, "qos": "'$MQTT_QOS'", "'$RETAIN'", "device_class":"motion","state_topic":"'$MQTT_PREFIX'/'$TOPIC_MOTION'","name":"'$UNIQUE_NAME'","unique_id":"'$UNIQUE_ID'","payload_on":"'$MOTION_START_MSG'","payload_off":"'$MOTION_STOP_MSG'"}'
$YI_HACK_PREFIX/bin/mosquitto_pub -i $HOSTNAME $RETAIN -h $HOST -t $TOPIC -m "$CONTENT"
# Human Detection
